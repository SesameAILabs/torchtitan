...
_TRITON_OK = False
try:
    import triton  # may fail if Triton not installed
    import triton.language as tl

    #  a) torch needs to be built with CUDA **and**
    #  b) at least one GPU is visible **and**
    #  c) Triton driver can be created
    if torch.cuda.is_available():
        # Touching `.active` raises RuntimeError if no devices / driver
        _TRITON_OK = True
...
def _rms_norm_ref(x: torch.Tensor, weight: torch.Tensor, eps: float = 1e-6):
    rstd = torch.rsqrt(x.pow(2).mean(-1, keepdim=True) + eps)  # 1/√(mean(x²)+eps)
...
if _TRITON_OK:
    ...
    class TritonFusedRMSNorm(torch.autograd.Function):
        ...
        def backward(ctx, dy):
            ...
            sm_count = device_module.get_device_properties(
                x.device
    ...
    fused_rms_norm_fn = _rms_norm_ref  # type: ignore
